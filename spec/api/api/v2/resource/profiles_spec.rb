...
describe 'API::V2::Resource::Profiles' do
  ...
  context 'event API behavior' do
    ...
    it 'receive model.profile.created notify' do
      expect(Barong::EventAPI).to receive(:notify).ordered.with('model.user.created', hash_including(:record))
      expect(Barong::EventAPI).to receive(:notify).ordered.with('model.profile.created', hash_including(:record))
    ...
    it 'receive model.profile.created notify with user and profile params' do
      expect(Barong::EventAPI).to receive(:notify).ordered.with('model.user.created', hash_including(:record))
      expect(Barong::EventAPI).to receive(:notify).ordered.with(
