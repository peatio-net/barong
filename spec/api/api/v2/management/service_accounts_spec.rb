    ...
    context 'when uid is present' do
      ...
      context 'when email is bad' do
        ...
        it 'renders an error' do
          ...
          expect_status_to_eq 422
          expect_body.to eq(error: ['Email does not appear to be a valid email address'])
        end
      end
      ...
      context 'when uid is not uniq, the same as user one' do
        ...
        it 'renders an error' do
          expect { do_request }.to_not change { ServiceAccount.count }
          expect_status_to_eq 422
          expect(json_body[:error].first).to include 'Email does not appear to be a valid email address'
        end
      end
    end
