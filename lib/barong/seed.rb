  ...
  class Seed
    ...
    def seed_permissions
      ...
      seeds['permissions'].each do |perm|
        logger.info '---'
        if Permission.find_by(role: perm['role'], verb: perm['verb'], path: perm['path'],
                              action: perm['action']).present?
    ...
    def seed_users
      ...
      seeds['users']&.each do |seed|
        ...
        if user.save
          ...
          if levels.count < user.level
            raise ConfigError.new("No enough levels found in database to grant the user to level #{user.level}")
          end
    ...
    def seed_restrictions
      ...
      seeds['restrictions'].each do |seed|
        logger.info '---'
        if Restriction.find_by(category: seed['category'], scope: seed['scope'], value: seed['value'],
                               state: seed['state']).present?
