    ...
    module Management
      ...
      private
        ...
        def check_request_method!
          ...
          raise Exceptions::Authentication.new(message: 'Only POST and PUT verbs are allowed.', status:  405)
        end

        def check_query_parameters!
          return if request.GET.empty?
          raise Exceptions::Authentication.new(message: 'Query parameters are not allowed.', status:  400)
        end

        def check_content_type!
          return if request.content_type == 'application/json'
          raise Exceptions::Authentication.new(message: 'Only JSON body is accepted.', status:  400)
        end
        ...
        def check_jwt!(jwt)
          begin
            ...
            raise Exceptions::Authentication.new(message: 'Failed to verify JWT.', debug_message: e.inspect, status: 401)
          end

          unless (scope.fetch(:mandatory_signers) - result[:verified]).empty?
            raise Exceptions::Authentication.new(message: 'Not enough signatures for the action.', status:  401)
          end
